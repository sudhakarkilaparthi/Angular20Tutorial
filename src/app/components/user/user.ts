...
export class User implements OnInit {
  ...
  resultSum = this.masterService.getSum(2, 5);
  ...
  OnUpdateUser() {
    ...
    this.http
      .put(
        'https://api.freeprojectapi.com/api/GoalTracker/updateUser?id=' + this.userObj.userId,
        this.userObj
      )
      .subscribe({
        next: (result) => {
  ...
  OnDeleteUser(userId: number) {
    ...
    this.http
      .delete('https://api.freeprojectapi.com/api/GoalTracker/deleteUser?id=' + userId)
      .subscribe({
        next: (result) => {
